...
pipeline {
    ...
    stages {
        stage('Test') {
            steps {
                git branch: 'sonatype', url: 'http://10.250.14.1:8929/root/spring-grade-test'
	...
	stage('publish') {
	    steps {
		withCredentials([usernamePassword(credentialsId: 'sonatype-nexus', usernameVariable: 'USERNAME', passwordVariable: 'PASSWORD')]) {
		    sh './gradlew publish'
  		}
	    }
	}
